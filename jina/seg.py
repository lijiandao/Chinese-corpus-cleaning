...
速率限制
速率限制以两种方式跟踪：RPM（每分钟请求数）和TPM（每分钟词元数）。限制是针对每个 IP/API 密钥强制执行的，并且可以根据首先达到的阈值（RPM 或 TPM）来达到。请注意，当请求中提供了 API 密钥时，速率限制是针对每个密钥而不是每个 IP 地址进行跟踪的。
搜索
列
列
	读取器 API	https://r.jina.ai	将 URL 转换为大模型友好文本	20 RPM	200 RPM	2000 RPM	4.6s	以输出响应中的词元数量为准。	GET/POST
	读取器 API	https://s.jina.ai	搜索网络并将结果转换为大模型友好文本		40 RPM	400 RPM	5.7s	每个请求都需要固定数量的词元，从 10000 个词元开始	GET/POST
	切分器 API	https://api.jina.ai/v1/segment	对长文本进行分词分句	20 RPM	200 RPM	1,000 RPM	0.3s	词元不计算使用量。	GET/POST
	深度搜索	https://deepsearch.jina.ai/v1/chat/completions	推理、搜索和迭代以找到最佳答案	2 RPM	10 RPM	100 RPM	56.7s	统计整个过程中词元的总数。	POST
	向量模型API	https://api
"""
    print('\n'.join(seg_sentence(content=content)))
